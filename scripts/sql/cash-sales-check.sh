#!/usr/bin/env bash
# Money cannot be parked on a customer that is not there. See cash-sales-check.sql.
set -u
...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/cash-sales-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -16
  exit 1
fi
echo "  a cash sale reaches the customer who bought it, and no division total moves"
