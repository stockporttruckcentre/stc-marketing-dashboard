...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/analytics-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -30
  exit 1
fi
echo "$out" | grep 'NOTICE:' | sed 's/^.*NOTICE:  /  /'
echo "  both readers are gated, only an administrator moves the line, and the comparison is a different window"
