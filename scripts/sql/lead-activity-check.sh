#!/usr/bin/env bash
# A lead that moves says so, and nothing else does.
set -u
...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/lead-activity-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -20
  exit 1
fi
echo "$out" | grep 'NOTICE:' | sed 's/^.*NOTICE:  /  /'
echo "  last updated means it: every field that says something moves it, bookkeeping does not"
