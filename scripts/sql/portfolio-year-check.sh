#!/usr/bin/env bash
# A portfolio against last year and against its target. See portfolio-year-check.sql.
set -u
...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/portfolio-year-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -16
  exit 1
fi
echo "  a portfolio reads against last year on both bases, and against its target"
