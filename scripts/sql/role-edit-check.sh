...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/role-edit-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -30
  exit 1
fi
echo "$out" | grep 'NOTICE:' | sed 's/^.*NOTICE:  /  /'
echo "  a refusal names a role and can be asked, a lead reaches only their own departments, and a red account reaches whoever runs sales"
