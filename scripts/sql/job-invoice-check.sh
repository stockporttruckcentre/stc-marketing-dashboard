#!/usr/bin/env bash
# Jobs, their invoices, and the wait between. See job-invoice-check.sql.
set -u
...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/job-invoice-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -16
  exit 1
fi
echo "  a job is open, or waiting on an invoice, or settled by one, and the invoice finds it"
