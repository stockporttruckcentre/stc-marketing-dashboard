#!/usr/bin/env bash
# Money cannot be parked on a customer that is not there. See report-customers-check.sql.
set -u
...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/report-customers-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -16
  exit 1
fi
echo "  reports rank the customer, not the billing name, and compare like with like"
