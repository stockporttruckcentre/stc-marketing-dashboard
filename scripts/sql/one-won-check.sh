#!/usr/bin/env bash
# One status for won, and winning is what makes somebody a customer.
set -u
...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/one-won-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -16
  exit 1
fi
echo "  one status for won, and winning is what makes somebody a customer"
