#!/usr/bin/env bash
# Every tracker entry belongs to its customer, however it is attached.
set -u
...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/tracker-entries-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -16
  exit 1
fi
echo "  every tracker entry reaches its customer: by record, by merge, by twin and by name"
