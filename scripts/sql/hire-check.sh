#!/usr/bin/env bash
# One price across a list, and the vendor behind a deal. See hire-check.sql.
set -u
...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/hire-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A4 "ERROR" | head -20
  exit 1
fi
echo "  a dry run writes nothing, a price reaches every unit named and no others, and the vendor rates fall back the right way"
