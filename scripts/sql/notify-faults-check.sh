#!/usr/bin/env bash
# Money cannot be parked on a customer that is not there. See notify-faults-check.sql.
set -u
...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/notify-faults-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -16
  exit 1
fi
echo "  the sweep cannot roll itself back, and one task is never counted twice"
