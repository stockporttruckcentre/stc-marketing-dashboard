#!/usr/bin/env bash
# Money cannot be parked on a customer that is not there. See misbound-check.sql.
set -u
...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/misbound-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -16
  exit 1
fi
echo "  an account cannot be bound to a merged away customer, and the ones that were are put back"
