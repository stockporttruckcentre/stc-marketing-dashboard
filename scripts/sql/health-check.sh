...
out=$(psql -p 55432 -U postgres -d stctest -q -f scripts/sql/health-check.sql 2>&1)
if echo "$out" | grep -q "ERROR"; then
  echo "$out" | grep -B2 -A2 "ERROR" | head -30
  exit 1
fi
echo "$out" | grep 'NOTICE:' | sed 's/^.*NOTICE:  /  /'
echo "  red tells them, amber waits for the button, and the cooldown counts working days"
