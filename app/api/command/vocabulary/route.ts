 ...
 * RLS lets them see and nothing else. Two people get two different
 * responses here, and that is the invariant rather than an accident:
 * the authoritative vocabulary is whatever one actor's own session can
 * see. The server side cache is keyed by user for the same reason, and
 * there is no shared half.
 *
